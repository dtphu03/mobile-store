						...
						<table class="table-cart-checkout mytable table-convert-price">
							...
							<c:forEach var="sanpham" items="${cart}" varStatus="loop">
								<tr class="cart_line" id="item${sanpham.id}">
									<div>
										<td>${loop.index+1}</td>
										<td><img src="/mobilestore/img/${sanpham.id}.png" style="width: 150px"
												class="cart-img"></td>
							...
							<tr>
								...
								<c:if test="${checkEmpty == 0 }">
									<td colspan="3">
										<a class="btn btn-primary" href="<%=request.getContextPath()%>/">Mua thêm sản
											phẩm khác</a>
									</td>
								</c:if>
								...
								<td colspan="4">
									<p class="cart_tongdonhang">Tổng giá trị đơn hàng : <span id="ordertotal"></span>
										VND</p>
								</td>
							</tr>
